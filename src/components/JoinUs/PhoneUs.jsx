import React, { useState } from 'react';

const PhoneUs = () => {
  const [isHovered, setIsHovered] = useState(false);

  // Phone SVG Icon
  const PhoneIcon = () => (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 24 24"
      width="24"
      height="24"
      className={`transition-colors duration-300 ${isHovered ? 'text-white' : 'text-red-500'}`}
    >
      <path
        fill="currentColor"
        d="M20.487 17.14l-4.065-3.696a1.001 1.001 0 00-1.391.043l-2.393 2.461c-.576-.11-1.734-.471-2.926-1.66-1.192-1.193-1.553-2.354-1.66-2.926l2.459-2.394a1 1 0 00.043-1.391L6.859 3.513a1 1 0 00-1.391-.087l-3.33 2.628a1 1 0 00-.29.649c-.015.25-.301 6.172 4.291 10.766C11.305 20.707 16.323 21 17.705 21c.202 0 .326-.006.359-.008a.992.992 0 00.648-.291l2.628-3.33a.997.997 0 00-.085-1.391z"
      />
    </svg>
  );

  return (
    <div
      className="fixed right-4 bottom-26 z-50 md:hidden" 
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
    >
      <a
        href="tel:+919880778585"
        className={`
          flex items-center justify-center
          w-13 h-13
          bg-black border border-black rounded-full
          cursor-pointer
          shadow-md
          no-underline
          transition-transform duration-300
          ${isHovered ? 'scale-110' : 'scale-100'}
        `}
        target="_blank"
        rel="noreferrer"
        aria-label="Call us"
      >
        <PhoneIcon />
      </a>
    </div>
  );
};

export default PhoneUs;